...
本家 Getting Started 全 8 章を**章ごとのファイル**に分けています。各パートの索引から章を選んでください（全 8 部・全章を移植済み。Lite 未対応の 8-03「VR の世界へ」のみ解説だけで、動くサンプルはありません）。
...
導入章 8 つを除く 35 章の内訳です（本家 Getting Started 全 8 章基準）。

| 判定 | 章数 | 割合 | 主な内容 |
|---|:--:|:--:|---|
| ○ 対応可 | 23 | 66% | 造形・変換・マテリアル・ライト・影・カメラ・親子関係・スケルタルアニメ・地形・スカイボックス・オーディオ |
| △ 条件付き | 11 | 31% | `MergeMeshes` / `FollowCamera` / `movePOV` / box の `faceUV` の欠如、パーティクル（NPE）・GUI（HTML 側）など作り方が異なるもの |
| ✕ 未対応 | 1 | 3% | WebXR / VR（8-03） |

チュートリアルの中核はほぼ再現できます。純粋に詰まるのは WebXR（8-03）のみで、パーティクルは NPE、音声は AudioV2 で対応します。
