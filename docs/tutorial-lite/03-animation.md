...
追加 import：`createBox, createSphere, onBeforeRender`

```typescript
const parent = createBox(engine, { size: 1 });
...
追加 import：`createBox, createCylinder`

```typescript
// ボディ
const body = createBox(engine, { width: 2, height: 0.5, depth: 1 });
body.position.y = 0.5;
